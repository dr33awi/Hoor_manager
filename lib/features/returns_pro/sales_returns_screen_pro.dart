// ═══════════════════════════════════════════════════════════════════════════
// Sales Returns Screen Pro - Professional Design System
// Modern Sales Returns Management Interface
// ═══════════════════════════════════════════════════════════════════════════
...
class SalesReturnsScreenPro extends ConsumerStatefulWidget {
  const SalesReturnsScreenPro({super.key});

  @override
  ConsumerState<SalesReturnsScreenPro> createState() =>
      _SalesReturnsScreenProState();
}

class _SalesReturnsScreenProState extends ConsumerState<SalesReturnsScreenPro> {
  ...
  Widget build(BuildContext context) {
    final returnsAsync = ref.watch(salesReturnsStreamProvider);
    ...
    if (_searchQuery.isNotEmpty) {
      filtered = filtered.where((r) {
        return r.invoiceNumber.contains(_searchQuery) ||
            (r.customerId?.toLowerCase().contains(_searchQuery.toLowerCase()) ??
                false);
      }).toList();
    }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: AppSpacing.sm),
            Text(
              'سجل مرتجعات المبيعات سيظهر هنا',
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // Select Invoice
                Text('اختر فاتورة المبيعات', style: AppTypography.labelLarge),
                SizedBox(height: AppSpacing.sm),
                invoicesAsync.when(
                  ...
                  data: (invoices) {
                    final salesInvoices = invoices
                        .where(
                            (i) => i.type == 'sale' && i.status != 'returned')
                        .toList();
                    return Container(
                      padding: EdgeInsets.symmetric(horizontal: AppSpacing.md),
                      decoration: BoxDecoration(
    ...
    try {
      ...
      await invoiceRepo.createInvoice(
        ...
        notes: reason.isEmpty
            ? 'مرتجع مبيعات - فاتورة رقم: ${invoice.invoiceNumber}'
            : reason,
      );
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              if (returnInvoice.customerId != null) ...[
                SizedBox(height: AppSpacing.sm),
                Container(
                  padding: EdgeInsets.all(AppSpacing.sm),
                  decoration: BoxDecoration(
